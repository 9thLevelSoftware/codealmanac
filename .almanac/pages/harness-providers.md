...
The main current cohesion risk in the Codex adapter is `[[src/harness/providers/codex/events.ts]]`. A 2026-05-15 codebase-smell review identified that file as owning legacy JSONL event mapping, app-server notification mapping, structured tool display shaping, actor/subagent tracing, usage parsing, and failure classification. Those jobs are related because they all translate Codex protocol data into `HarnessEvent` and `HarnessResult`, but protocol drift forces a maintainer to reason through too many concerns in one file. The preferred cleanup is to split notification mapping, display mapping, actor trace state, usage parsing, and failure classification into named Codex-provider modules while keeping the public harness contract unchanged.
